# Welcome to GitHub Desktop!

ריפו עם פרויקטים קטנים וכיפיים.

## 🍕 משחק: צבי נינג'ה והפיצות

משחק מגע לאייפד לגיל 4+ — תפסו פיצות עם צבי הנינג'ה!

**לשחק באייפד:** https://guyzom.github.io/desktop-tutorial/

> **הקישור לא עובד?** פעם אחת: [Settings → Pages](https://github.com/guyzom/desktop-tutorial/settings/pages) → Source: **GitHub Actions** → ואז Re-run ב-[Actions](https://github.com/guyzom/desktop-tutorial/actions).

פתיחה מקומית: [`pizza-ninjas/index.html`](./pizza-ninjas/index.html)

פרטים: [`pizza-ninjas/README.md`](./pizza-ninjas/README.md)
